...
function composeOilChange(p: BrandProfile): Composed {
  return {
    ...
    faqs: [
      ...
      { question: `Which oil grade does my ${p.brandName} need?`, answer: `Your ${p.brandName} requires ${p.oilSpec}. Fitting a non-approved oil, even one of the correct viscosity, can trip long-term wear protection and in some cases invalidate warranty coverage.` },
      ...
      { question: `Does an independent oil change void the ${p.brandName} warranty?`, answer: 'No, provided we use approved fluids and parts and document the work, which we always do. UAE consumer law protects your right to service outside the dealer as long as spec is met.' },
    ],
  };
}
...
function composeTransmission(p: BrandProfile): Composed {
  return {
    ...
    partsCopy: `We only fit ${p.transmissionFluid}: this is not a place for generic ATF. Filters, gaskets, and pan bolts are genuine ${p.brandName} or approved OE-supplier. Mechatronic units and torque converters, when needed, are supplied factory-remanufactured with warranty.`,
    faqs: [
      ...
      { question: `Can you rebuild a ${p.brandName} gearbox?`, answer: `Yes. From clutch-pack replacement to mechatronic unit remanufacture and full rebuild, we handle the ${p.transmissionName} end to end in-house, with a warranty on the completed work.` },
...
function composeDiagnostics(p: BrandProfile): Composed {
  return {
    ...
    faqs: [
      ...
      { question: `Do you keep records of ${p.brandName} diagnostic sessions?`, answer: 'Yes. Every scan report is saved against your VIN so return visits and future owners have a complete diagnostic history, valuable for warranty claims and resale.' },
    ],
  };
}
