...
const LocalGaragePage = () => {
  ...
  const page = sourcePage && isArabic ? (() => {
    ...
    const faqs = [
      { question: 'أين تقع ديجي-تك؟', answer: 'نقع في القوز الصناعية 3، مستودع 11-15، دبي.' },
  ...
  const url = `${SITE_URL}${isArabic ? '/ar' : ''}/services/${page.slug}`;
  const whatsapp = `https://wa.me/97143402223?text=${encodeURIComponent(isArabic ? `مرحباً ديجي-تك، أحتاج مساعدة بخصوص ${page.title}.` : `Hi DIGI-TEC, I need help with ${page.title.toLowerCase()}.`)}`;
