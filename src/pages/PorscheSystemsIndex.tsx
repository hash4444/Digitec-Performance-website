...
const PorscheSystemsIndex = () => {
  ...
  useSeo({ title: 'Porsche Systems Explained | Digi-Tec Dubai', description: 'Understand Porsche PDK, Tiptronic, PASM, PDCC, PCCB, air suspension, Sport Chrono, rear-axle steering and PTM systems.', canonical: url, jsonLd: pageGraph([buildWebPage({ url, type: 'CollectionPage', mainEntityId: `${url}#list`, name: 'Porsche Systems Explained', description: 'Technical Porsche system guides for owners.', breadcrumbId: `${url}#breadcrumb` }), buildBreadcrumb(url, [{ name: 'Home', url: SITE_URL }, { name: 'Porsche', url: `${SITE_URL}${PORSCHE_HUB_PATH}` }, { name: 'Systems', url }]), { '@type': 'ItemList', '@id': `${url}#list`, name: 'Porsche system guides', numberOfItems: porscheSystemGuides.length, itemListElement: porscheSystemGuides.map((g, i) => ({ '@type': 'ListItem', position: i + 1, name: g.name, item: `${SITE_URL}${PORSCHE_SYSTEMS_PATH}/${g.slug}` })) }]), hasArabicVersion: false });
